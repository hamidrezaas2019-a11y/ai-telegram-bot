...
async function novaClearHistory(db, targetId) {
    await db
        .prepare(`
            DELETE FROM conversations
            WHERE telegram_id = ?
...
function novaIsAdmin(env, telegramId) {
...
async function novaUpdateUser(
    ...
    for (
        const key of Object.keys(changes)
    ) {
...
async function novaClearHistory(
    db,
    telegramId
) {
...
async function novaRootRoute(
    context
) {

    const runtime =
        await novaRuntimeStatus(
            context.env
        );
...
async function novaHealthRoute(
    context
) {
...
async function novaHistoryRoute(
    context
) {
...
async function novaHandleApiRequest(
    request,
    env
) {

    const context =
        novaCreateRequestContext(
            request,
            env
        );
