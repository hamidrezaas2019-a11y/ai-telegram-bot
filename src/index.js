...
// ============================================
// دیتابیس D1 - نسخه کامل و بدون تکرار
// ============================================
...
export const DB = {
  ...
  // ===== مدل‌های پیش‌فرض (فقط ۳ مدل واقعی) =====
  async setDefaultModels(env) {
    ...
    for (const model of models) {
      await env.DB.prepare(`
        INSERT OR IGNORE INTO models (model_id, display_name, category, provider, cost_text, cost_image, level)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).bind(
        model.id,
        model.name,
  ...
  async setDefaultSubscriptions(env) {
    const subscriptions = [
      { name: 'تست ۱ روزه', price: 15000, days: 1, features: '{"text":50,"image":20,"voice":10}' },
      { name: 'بسته تصویر', price: 49000, days: 0, features: '{"text":50,"image":100}' },
      { name: 'بسته ویدیو', price: 79000, days: 0, features: '{"text":50,"video":50}' },
  ...
  async setDefaultSliders(env) {
    const sliders = [
      { title: 'اشتراک ویژه', subtitle: 'دسترسی به تمام مدل‌ها', button: 'خرید اشتراک', bg: 'color', val: '#667eea', order: 1 },
      { title: 'تولید تصویر', subtitle: 'با بهترین مدل‌ها', button: 'مشاهده مدل‌ها', bg: 'image', val: '/assets/slider2.jpg', order: 2 },
      { title: 'چت پیشرفته', subtitle: 'با بیش از ۴۰ مدل', button: 'شروع چت', bg: 'color', val: '#764ba2', order: 3 }
    ];
