...
// ============================================================
// 🚀 PART 8: MAIN WORKER (Lines 3701-3900)
// ============================================================
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path === '/api/user') {
        ...
        const balance = await db.getBalance(body.user_id);
        const premium = await db.checkPremium(body.user_id);
        return new Response(JSON.stringify({ ...user, ...balance, premium }), {
          headers: { 'Content-Type': 'application/json' }
        });
      }
      ...
      // ---- API: Models ----
      if (path === '/api/models') {
        return new Response(JSON.stringify(CONFIG.MODELS), {
          headers: { 'Content-Type': 'application/json' }
        });
      }
      ...
      // ---- Set Webhook ----
      if (path === '/setwebhook') {
        const result = await telegram.setWebhook(CONFIG.WEBHOOK_URL, {
          dropPending: true,
          maxConnections: 100,
          allowedUpdates: ['message', 'callback_query', 'inline_query', 'pre_checkout_query', 'shipping_query']
        });
      ...
      return new Response(`🤖 AI Bot v${CONFIG.VERSION} is running!\n📦 ${Object.values(CONFIG.MODELS).reduce((s, m) => s + Object.keys(m).length, 0)} models loaded.`, { 
        status: 200,
