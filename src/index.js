...
const CONFIG = {
  ...
  // ---- Database ----
DATABASE_URL: 'postgresql://neondb_owner:npg_NBR91MYPaokx@ep-silent-breeze-ax41xz48-pooler.c-4.us-east-2.aws.neon.tech/neondb?sslmode=require',
...
// ============================================================
// 🗄️ DATABASE CLASS - Pure Fetch (No Dependencies)
// ============================================================
...
class Database {
  ...
  // ============================================================
  // ✅ QUERY - اصلاح شده (خطا رو throw نمیکنه)
  // ============================================================
  ...
  // ============================================================
  // ✅ MIGRATE - اصلاح شده (با تست اتصال)
  // ============================================================
  async migrate() {
    try {
...
// ============================================================
// 🚀 PART 8: MAIN WORKER (FIXED — uses env secrets)
// ============================================================
...
export default {
  async fetch(request, env, ctx) {
    ...
    // این‌ها باید با `wrangler secret put NAME` ست شده باشند
    const runtimeConfig = {
      ...CONFIG,
    ...
    try {
      const db = new Database(runtimeConfig.DATABASE_URL);
      ...
      if (path === '/setwebhook') {
        ...
        try {
          ...
          if (!botToken) {
            return new Response(JSON.stringify({ success: false, error: 'BOT_TOKEN not configured. Run: wrangler secret put BOT_TOKEN' }), {
              status: 400,
              headers: { 'Content-Type': 'application/json' }
            });
          }
          ...
          const result = await response.json();
          return new Response(JSON.stringify({ success: result.ok, description: result.description, webhook: webhookUrl }, null, 2), {
      ...
      if (path === '/api/user') {
        ...
        try {
          ...
          return new Response(JSON.stringify({ ...user, ...balance, premium, isPremium: premium }), {
            status: 200,
            headers: { 'Content-Type': 'application/json' }
          });
        } catch (error) {
      ...
      if (path === '/api/stats') {
        ...
        try {
          ...
          return new Response(JSON.stringify({ ...stats, models: totalModels, version: CONFIG.VERSION }), {
            status: 200,
            headers: { 'Content-Type': 'application/json' }
          });
        } catch (error) {
      ...
      if (path === '/api/models') {
        ...
        const totalModels = Object.values(CONFIG.MODELS).reduce((sum, m) => sum + Object.keys(m).length, 0);
        return new Response(JSON.stringify({ models: CONFIG.MODELS, total: totalModels, categories: Object.keys(CONFIG.MODELS) }), {
          status: 200,
      ...
      if (path === '/migrate') {
        ...
        try {
          ...
          return new Response(JSON.stringify({ success: result, message: result ? '✅ Migration completed' : '❌ Migration failed' }), {
      ...
      if (path === '/cleanup') {
        ...
        try {
          ...
          return new Response(JSON.stringify({ success: result, message: result ? '✅ Cleanup completed' : '❌ Cleanup failed' }), {
            status: result ? 200 : 500,
            headers: { 'Content-Type': 'application/json' }
          });
        } catch (error) {
