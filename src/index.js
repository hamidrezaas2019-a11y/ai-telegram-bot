...
// ============================================================
// 🔐 PART 1: CONFIGURATION (کامل و یکپارچه)
// ============================================================
...
const CONFIG = {
  ...
  WEBHOOK_URL: 'https://ai-telegram-bot.hamidreza-as2019.workers.dev/webhook',
  MINIAPP_URL: 'https://ai-telegram-bot.hamidreza-as2019.workers.dev/miniapp',
...
}; // <-- اینجا CONFIG کامل بسته می‌شود

// ============================================================
// 🗄️ DATABASE CLASS - D1 (Cloudflare Native)
// ============================================================
...
class Database {
  ...
  async query(sqlText, params = []) {
    try {
      const cacheKey = sqlText + JSON.stringify(params);
      ...
      const result = await this.env.DB.prepare(sqlText).bind(...params).all();
  ...
  async migrate() {
    try {
      const test = await this.query('SELECT 1 as test');
      if (test.error) {
        console.error('❌ DB Connection failed:', test.message);
        return false;
      }
  ...
  async getUser(userId) {
    try {
      const result = await this.query('SELECT * FROM users WHERE user_id = ?', [userId]);
  ...
  async getUsers(limit = 100, offset = 0) {
    try {
      const result = await this.query('SELECT * FROM users ORDER BY created_at DESC LIMIT ? OFFSET ?', [limit, offset]);
      return result.rows || [];
    } catch (e) { return []; }
  }
  ...
  async ensureUser(userId, username, firstName, lastName) {
    try {
      ...
      if (!user) {
        ...
        await this.logActivity(userId, 'register', { method: 'telegram', username, firstName, lastName });
      }
      return user;
    } catch (e) { 
  ...
  async updateUser(userId, data) {
    try {
      ...
      await this.query(`UPDATE users SET ${setClause}, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?`, [...values, userId]);
      return true;
  ...
  async addBalance(userId, type, amount) {
    try {
      await this.query(`UPDATE users SET balance_${type} = balance_${type} + ? WHERE user_id = ?`, [amount, userId]);
      return true;
    } catch (e) { return false; }
  }
  ...
  async setPremium(userId, plan, days) {
    try {
      ...
      await this.query('UPDATE users SET premium_expiry = ?, premium_plan = ? WHERE user_id = ?', [expiry.toISOString(), plan, userId]);
  ...
  async addPremiumDays(userId, days) {
    try {
      ...
      await this.query('UPDATE users SET premium_expiry = ? WHERE user_id = ?', [expiry.toISOString(), userId]);
      return expiry;
    } catch (e) { return null; }
  }
  ...
  async getPayment(paymentId) {
    try {
      const result = await this.query('SELECT * FROM payments WHERE id = ?', [paymentId]);
      return result.rows?.[0] || null;
    } catch (e) { return null; }
  }
  ...
  async getPayments(userId, limit = 20) {
    try {
      const result = await this.query('SELECT * FROM payments WHERE user_id = ? ORDER BY created_at DESC LIMIT ?', [userId, limit]);
      return result.rows || [];
    } catch (e) { return []; }
  }
  ...
  async confirmPayment(paymentId) {
    try {
      ...
      await this.query('UPDATE payments SET status = \'confirmed\', confirmed_at = CURRENT_TIMESTAMP WHERE id = ?', [paymentId]);
  ...
  async rejectPayment(paymentId, reason = null) {
    try {
      await this.query('UPDATE payments SET status = \'rejected\', admin_note = ? WHERE id = ?', [reason, paymentId]);
      return true;
    } catch (e) { return false; }
  }
  ...
  async addReferral(referrerId, referredId) {
    try {
      await this.query('INSERT INTO referrals (referrer_id, referred_id, status) VALUES (?, ?, \'active\')', [referrerId, referredId]);
      await this.query('UPDATE users SET invite_count = invite_count + 1 WHERE user_id = ?', [referrerId]);
  ...
  async getReferrals(userId) {
    try {
      const result = await this.query('SELECT COUNT(*) as count FROM referrals WHERE referrer_id = ? AND status = \'active\'', [userId]);
  ...
  async getTickets(userId, limit = 20) {
    try {
      const result = await this.query('SELECT * FROM tickets WHERE user_id = ? ORDER BY created_at DESC LIMIT ?', [userId, limit]);
      return result.rows || [];
    } catch (e) { return []; }
  }
  ...
  async respondTicket(ticketId, adminId, response) {
    try {
      await this.query('UPDATE tickets SET admin_response = ?, admin_id = ?, status = \'answered\' WHERE id = ?', [response, adminId, ticketId]);
      return true;
    } catch (e) { return false; }
  }
  ...
  async closeTicket(ticketId) {
    try {
      await this.query('UPDATE tickets SET status = \'closed\', closed_at = CURRENT_TIMESTAMP WHERE id = ?', [ticketId]);
      return true;
    } catch (e) { return false; }
  }
  ...
  async logActivity(userId, action, details = null) {
    try {
      await this.query('INSERT INTO activity_logs (user_id, action, details) VALUES (?, ?, ?)', [userId, action, JSON.stringify(details || {})]);
    } catch (e) {}
  }

  async getActivityLogs(userId, limit = 50) {
    try {
      const result = await this.query('SELECT * FROM activity_logs WHERE user_id = ? ORDER BY created_at DESC LIMIT ?', [userId, limit]);
      return result.rows || [];
    } catch (e) { return []; }
  }
  ...
  async getStats() {
    try {
      ...
      const premium = await this.query('SELECT COUNT(*) as count FROM users WHERE premium_expiry > CURRENT_TIMESTAMP');
  ...
  async getSession(userId) {
    try {
      const result = await this.query('SELECT session_data FROM sessions WHERE user_id = ? AND expires_at > CURRENT_TIMESTAMP', [userId]);
  ...
  async getModel(modelId, category) {
    try {
      const result = await this.query('SELECT * FROM ai_models WHERE model_id = ? AND category = ?', [modelId, category]);
      return result.rows?.[0] || null;
    } catch (e) { return null; }
  }
  ...
  async getAllModels() {
    try {
      const result = await this.query('SELECT * FROM ai_models ORDER BY category, level');
      return result.rows || [];
    } catch (e) { return []; }
  }
  ...
  async getModelsByCategory(category) {
    try {
      const result = await this.query('SELECT * FROM ai_models WHERE category = ? ORDER BY level', [category]);
      return result.rows || [];
    } catch (e) { return []; }
  }
  ...
  async getModelUsage(userId, modelId, category) {
    try {
      const result = await this.query(
        'SELECT COUNT(*) as count FROM model_usage WHERE user_id = ? AND model_id = ? AND category = ? AND created_at > CURRENT_TIMESTAMP - INTERVAL \'1 day\'',
  ...
  async trackModelUsage(userId, modelId, category, success = true) {
    try {
      await this.query('INSERT INTO model_usage (user_id, model_id, category, success) VALUES (?, ?, ?, ?)', [userId, modelId, category, success]);
      const countField = `${category}_count`;
      await this.query(`UPDATE users SET ${countField} = ${countField} + 1 WHERE user_id = ?`, [userId]);
    } catch (e) {}
  }
  ...
  async getSettings(group = 'general') {
    try {
      const result = await this.query('SELECT * FROM settings WHERE setting_group = ?', [group]);
      return result.rows || [];
    } catch (e) { return []; }
  }
  ...
  async banUser(userId, reason = null) {
    try {
      await this.query('UPDATE users SET is_banned = TRUE WHERE user_id = ?', [userId]);
  ...
  async unbanUser(userId) {
    try {
      await this.query('UPDATE users SET is_banned = FALSE WHERE user_id = ?', [userId]);
  ...
  async getBannedUsers() {
    try {
      const result = await this.query('SELECT * FROM users WHERE is_banned = TRUE ORDER BY updated_at DESC');
      return result.rows || [];
    } catch (e) { return []; }
  }
  ...
  async makeAdmin(userId) {
    try {
      await this.query('UPDATE users SET is_admin = TRUE WHERE user_id = ?', [userId]);
  ...
  async removeAdmin(userId) {
    try {
      await this.query('UPDATE users SET is_admin = FALSE WHERE user_id = ?', [userId]);
  ...
  async getAdmins() {
    try {
      const result = await this.query('SELECT * FROM users WHERE is_admin = TRUE ORDER BY created_at');
      return result.rows || [];
    } catch (e) { return []; }
  }
  ...
  async getPremiumUsers() {
    try {
      const result = await this.query('SELECT COUNT(*) as count FROM users WHERE premium_expiry > CURRENT_TIMESTAMP');
      return result.rows?.[0]?.count || 0;
    } catch (e) { return 0; }
  }
...
// ============================================================
// 🎯 PART 6: MAIN HANDLERS (FIXED - با this.config)
// ============================================================

class BotHandlers {
  constructor(db, telegram, ai, config) {  // ✅ config رو اضافه کردم
    this.db = db;
    this.telegram = telegram;
    this.ai = ai;
    this.config = config;  // ✅ ذخیره کردم
  ...
  async handleStart(chatId, user, args = '') {
    // ✅ اصلاح: استفاده از this.config
    const gift = this.config.WELCOME_GIFT;
...
👥 **زیرمجموعه:**
• هر دعوت ${this.config.REFERRAL_BONUS.text} اعتبار متن
• ${this.config.REFERRAL_BONUS.image} اعتبار تصویر
• ${this.config.REFERRAL_BONUS.voice} اعتبار صدا
• ${this.config.REFERRAL_BONUS.video} اعتبار ویدیو
• ${this.config.REFERRAL_BONUS.premium_days || 0} روز اشتراک ویژه
...
📱 **مینی‌اپ:** ${this.config.MINIAPP_URL}
`;
    ...
    if (args && args.startsWith('ref_')) {
      ...
      if (referrerId && referrerId !== user.user_id) {
        ...
        await this.telegram.sendMessage(chatId, `🎉 شما با دعوت نامه ثبت نام کردید!\nبه ${this.config.REFERRAL_BONUS.text} اعتبار متن و ${this.config.REFERRAL_BONUS.premium_days || 0} روز اشتراک ویژه دسترسی پیدا کردید.`);
      }
    }
...
تحلیل را به فارسی و جامع بنویسید.`;
  ...
  async handleSearch(chatId, query, user, options = {}) {
    ...
    try {
      const apiKey = this.config.TAVILY_API_KEY;
      if (!apiKey) {
...
`);
  ...
  async showSubscriptionPlans(chatId) {
    // ✅ اصلاح: استفاده از this.config
    const plans = this.config.SUBSCRIPTION_PLANS;
...
`;
  ...
  async handleSubscriptionPurchase(chatId, user, planId) {
    // ✅ اصلاح: استفاده از this.config
    const plan = this.config.SUBSCRIPTION_PLANS[planId];
    if (!plan) {
...
`, KEYBOARDS.payment);
  ...
  async handlePaymentMethod(chatId, user, method) {
    ...
    const plan = this.config.SUBSCRIPTION_PLANS[session.pending_plan];
    if (!plan) {
...
`;
  ...
  async handleConfirmPayment(chatId, user) {
    ...
    // ✅ اصلاح: استفاده از this.config
    const plan = this.config.SUBSCRIPTION_PLANS[result.plan];
    if (plan && plan.days > 0) {
      await this.telegram.sendMessage(chatId, `
...
`, KEYBOARDS.main);
  ...
  async handleReferral(chatId, user) {
    // ✅ اصلاح: استفاده از this.config
    const bonus = this.config.REFERRAL_BONUS;
    const referrals = await this.db.getReferrals(user.user_id);
...
🔗 **لینک دعوت شما:**
\`https://t.me/${this.config.BOT_NAME || 'YourBot'}?start=ref_${user.user_id}\`

🎁 **پاداش هر دعوت:**
• 📝 ${bonus.text} اعتبار متن
• 🎨 ${bonus.image} اعتبار تصویر
• 🎵 ${bonus.voice} اعتبار صدا
• 🎬 ${bonus.video} اعتبار ویدیو
• ⭐ ${bonus.premium_days || 0} روز اشتراک ویژه
...
• هر دعوت ${bonus.text} اعتبار متن دارد
...
• شماره تیکت خود را یادداشت کنید`;
  ...
  async handleStats(chatId) {
    ...
    const totalModels = Object.values(this.config.MODELS)
...
🔄 **آخرین بروزرسانی:** ${this.config.VERSION}`;
...
`;
    ...
    for (const [cat, models] of Object.entries(this.config.MODELS)) {
    ...
    const total = Object.values(this.config.MODELS)
      .reduce((sum, m) => sum + Object.keys(m).length, 0);
    msg += `📊 **مجموع:** ${total} مدل`;
...
`);
  ...
  async handleCommand(text, chatId, user) {
    ...
    const isAdmin = user.is_admin || this.config.ADMIN_IDS.includes(user.user_id);
  ...
  async handleCallback(update) {
    try {
      ...
      const isAdmin = user.is_admin || this.config.ADMIN_IDS.includes(userId);
...
✅ **مدل ${this.config.MODELS[category]?.[modelId]?.name || modelId} انتخاب شد**

🤖 **دسته:** ${category}
🏢 **ارائه‌دهنده:** ${this.config.MODELS[category]?.[modelId]?.provider || 'نامشخص'}
...
⭐ **کاربران ویژه** به مدل‌های پیشرفته دسترسی دارند.`
      ...
      results.push({
        ...
        reply_markup: {
          inline_keyboard: [[
            { text: '⭐ مشاهده اشتراک‌ها', url: this.config.MINIAPP_URL || 'https://t.me/YourBot' }
          ]]
        }
      });
...
// ============================================================
// 🚀 PART 8: MAIN WORKER (FULL VERSION)
// ============================================================
...
async function sendTelegram(env, chatId, text, keyboard = null) {
...
class Database {
  ...
  async ensureUser(userId, username = null, firstName = null, lastName = null) {
    try {
      let user = await this.getUser(userId);
      ...
      if (!user) {
        ...
        `, [userId, username, firstName, lastName, inviteCode, 10, 5, 3, 2]);
        
        user = await this.getUser(userId);
        console.log(`✅ New user created: ${userId}`);
      } else {
        if (username && user.username !== username) {
          await this.query('UPDATE users SET username = ? WHERE user_id = ?', [username, userId]);
        }
        await this.query('UPDATE users SET last_activity = CURRENT_TIMESTAMP WHERE user_id = ?', [userId]);
      }
  ...
  async deductBalance(userId, type, amount = 1) {
    const balance = await this.getBalance(userId);
    if (!balance || balance[type] < amount) return false;
    await this.query(`UPDATE users SET balance_${type} = balance_${type} - ? WHERE user_id = ?`, [amount, userId]);
    return true;
  ...
  async logUsage(userId, model, type, prompt = null, success = true) {
    await this.query(
      'INSERT INTO usage_logs (user_id, model, type, prompt, success) VALUES (?, ?, ?, ?, ?)',
      [userId, model, type, prompt, success ? 1 : 0]
    );
  }

  async getStats() {
    const total = await this.query('SELECT COUNT(*) as count FROM users');
    const premium = await this.query("SELECT COUNT(*) as count FROM users WHERE premium_expiry > datetime('now')");
    return {
      totalUsers: total.rows?.[0]?.count || 0,
      premiumUsers: premium.rows?.[0]?.count || 0
    };
  }
  ...
  async cleanup() {
    await this.query("DELETE FROM usage_logs WHERE created_at < datetime('now', '-30 days')");
    await this.query("UPDATE payments SET status = 'expired' WHERE status = 'pending' AND created_at < datetime('now', '-7 days')");
    return true;
  }
}
...
class AIService {
  ...
  async chat(prompt, model = 'gemini-2.0-flash') {
...
class BotHandlers {
  constructor(db, telegram, ai, config) {
    this.db = db;
    this.telegram = telegram;
    this.ai = ai;
    this.config = config;
...
/subscription - اشتراک
`;
    await this.telegram.sendMessage(chatId, msg, KEYBOARDS.main);
  }

  async handleChat(chatId, prompt, user) {
    const isPremium = await this.db.isPremium(user.user_id);
    const balance = await this.db.getBalance(user.user_id);

    if (!isPremium && (balance?.text || 0) < 1) {
      await this.telegram.sendMessage(chatId, '❌ اعتبار متن کافی نیست! از /subscription استفاده کنید.');
      return;
    }
    ...
    try {
      ...
      if (!isPremium) {
        await this.db.deductBalance(user.user_id, 'text');
      }
      await this.db.logUsage(user.user_id, 'gemini', 'chat', prompt.substring(0, 100));

    } catch (error) {
      await this.telegram.sendMessage(chatId, `❌ خطا: ${error.message}`);
    }
  }

  async handleImage(chatId, prompt, user) {
    const isPremium = await this.db.isPremium(user.user_id);
    const balance = await this.db.getBalance(user.user_id);

    if (!isPremium && (balance?.image || 0) < 1) {
      await this.telegram.sendMessage(chatId, '❌ اعتبار تصویر کافی نیست! از /subscription استفاده کنید.');
      return;
    }
    ...
    try {
      const imageUrl = await this.ai.image(prompt);
      if (imageUrl) {
        await this.telegram.sendPhoto(chatId, imageUrl, `🎨 ${prompt}`);
        if (!isPremium) {
          await this.db.deductBalance(user.user_id, 'image');
        }
        await this.db.logUsage(user.user_id, 'dall-e', 'image', prompt.substring(0, 100));
      } else {
        await this.telegram.sendMessage(chatId, '❌ خطا در تولید تصویر');
      }
    } catch (error) {
      await this.telegram.sendMessage(chatId, `❌ خطا: ${error.message}`);
    }
  ...
  async handleBalance(chatId, user) {
    const balance = await this.db.getBalance(user.user_id);
    const isPremium = await this.db.isPremium(user.user_id);
...
💡 **نکات:**
• روزانه ۱۰ چت رایگان
• روزانه ۳ تصویر رایگان
• کاربران ویژه نامحدود
`;
    await this.telegram.sendMessage(chatId, msg);
  }

  async handleSubscription(chatId) {
    const msg = `
⭐ **بسته‌های اشتراک**

🔥 **حرفه‌ای** - ۹۹,۰۰۰ تومان
• ۵۰۰ چت
• ۲۰۰ تصویر
• ۱۰۰ صدا
• ۵۰ ویدیو
...
`;
  ...
  async handleWebhook(update) {
    if (update.message) {
      ...
      const user = await this.db.ensureUser(
        from.id,
        from.username,
        from.first_name,
        from.last_name
      ...
      if (!user) {
        await this.telegram.sendMessage(chatId, '❌ خطا در ثبت‌نام');
        return;
      }

      if (user.is_banned) {
        await this.telegram.sendMessage(chatId, '⛔ شما مسدود شده‌اید');
        return;
      }
      ...
      if (text.startsWith('/')) {
        ...
        switch (cmd) {
          case '/start':
            ...
            break;
          case '/chat':
            if (args) await this.handleChat(chatId, args, user);
            else await this.telegram.sendMessage(chatId, '❌ متن را وارد کنید: /chat سلام');
            break;
          case '/image':
            if (args) await this.handleImage(chatId, args, user);
            else await this.telegram.sendMessage(chatId, '❌ پرامپت را وارد کنید: /image یک گربه');
            break;
          case '/balance':
            await this.handleBalance(chatId, user);
            break;
          case '/help':
            await this.handleHelp(chatId);
            break;
          case '/subscription':
            await this.handleSubscription(chatId);
            break;
          default:
            await this.telegram.sendMessage(chatId, '❌ دستور ناشناخته. /help برای راهنما.');
        }
        ...
        // پیام معمولی = چت
        await this.handleChat(chatId, text, user);
      }
    }
    ...
    if (update.callback_query) {
      ...
      switch (data) {
        case 'chat':
          await this.telegram.sendMessage(chatId, '💬 متن خود را ارسال کنید:');
          break;
        case 'image':
          await this.telegram.sendMessage(chatId, '🎨 پرامپت تصویر را ارسال کنید:');
          break;
        case 'balance':
          await this.handleBalance(chatId, user);
          break;
        case 'help':
          ...
          break;
        default:
          await this.telegram.sendMessage(chatId, '⚡ گزینه نامعتبر');
      }
    }
  }
}
...
const KEYBOARDS = {
  main: {
    inline_keyboard: [
      [{ text: '💬 چت', callback_data: 'chat' }, { text: '🎨 تصویر', callback_data: 'image' }],
      [{ text: '💰 موجودی', callback_data: 'balance' }, { text: '📚 راهنما', callback_data: 'help' }]
    ]
  }
};
...
export default {
  async fetch(request, env, ctx) {
    ...
    const config = {
      ...
      ADMIN_IDS: env.ADMIN_IDS ? env.ADMIN_IDS.split(',').map(id => id.trim()) : [],
    };
    ...
    try {
      ...
      const handlers = new BotHandlers(db, telegram, ai, config);
      ...
      if (path === '/setwebhook') {
        ...
        return new Response(JSON.stringify({ success: result.ok, webhook: webhookUrl, result }, null, 2), {
          headers: { 'Content-Type': 'application/json' }
        });
      }
      ...
      if (path === '/health') {
        ...
        return new Response(JSON.stringify({
          ...
          database: dbStatus,
          botTokenConfigured: !!config.BOT_TOKEN,
          models: 2
        }, null, 2), {
          headers: { 'Content-Type': 'application/json' }
        });
...
📌 Available endpoints:
  ...
  GET  /migrate       - Run migration
  GET  /cleanup       - Cleanup database
  GET  /              - This page

💡 For help: /help in Telegram
`, {
