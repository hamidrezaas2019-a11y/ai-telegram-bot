...
// ============================================================
// 🚀 PART 8: MAIN WORKER (FIXED)
// ============================================================

// کلاس‌ها را برای استفاده در Worker صادر می‌کنیم
export { Database, AIService, TelegramService, BotHandlers, CONFIG };
...
export default {
  async fetch(request, env, ctx) {
    ...
    // 🔑 ترکیب CONFIG با مقادیر واقعی از Cloudflare Secrets
    const runtimeConfig = {
      ...CONFIG,
      ...
      MIDJOURNEY_API_KEY: env.MIDJOURNEY_API_KEY,
    ...
    // بررسی وجود BOT_TOKEN
    if (!runtimeConfig.BOT_TOKEN) {
      console.error('❌ BOT_TOKEN is not configured!');
      return new Response('❌ BOT_TOKEN is not configured. Please set it using: wrangler secret put BOT_TOKEN', {
        status: 500,
    ...
    try {
      // ✅ ایجاد نمونه‌های سرویس‌ها با استفاده از new
      const db = new Database(env);
      const telegram = new TelegramService(runtimeConfig.BOT_TOKEN);
      const ai = new AIService(runtimeConfig);
      const handlers = new BotHandlers(db, telegram, ai, runtimeConfig); // ✅ اصلاح شد
      ...
      if (path === '/' || path === '') {
        ...
        return new Response(`🤖 ULTIMATE AI BOT v${CONFIG.VERSION}\n📊 Status: Online\n🤖 Models: ${totalModels}\n📱 Mini App: ${CONFIG.MINIAPP_URL}`, {
          status: 200,
          headers: { 'Content-Type': 'text/plain; charset=utf-8' }
        });
      }
      ...
      if (path === '/health') {
        ...
        return new Response(JSON.stringify({
          ...
          models: totalModels,
          database: dbStatus,
          botTokenConfigured: !!runtimeConfig.BOT_TOKEN,
      ...
      if (path === '/cleanup') {
        ...
        try {
          ...
          return new Response(JSON.stringify({ 
            success: result, 
            message: result ? '✅ Cleanup completed' : '❌ Cleanup failed' 
          }), {
            status: result ? 200 : 500,
            headers: { 'Content-Type': 'application/json' }
          });
...
💡 For help: https://github.com/your-repo
  ...
  async scheduled(event, env, ctx) {
    ...
    try {
      // ✅ استفاده صحیح از new Database(env)
      const db = new Database(env);

      // پاکسازی جلسات منقضی شده
      await db.query(`DELETE FROM sessions WHERE expires_at < CURRENT_TIMESTAMP`);
      
      // به‌روزرسانی پرداخت‌های منقضی شده
      await db.query(`UPDATE payments SET status = 'expired' WHERE status = 'pending' AND created_at < CURRENT_TIMESTAMP - INTERVAL '1 day'`);
      
      // پاکسازی لاگ‌های قدیمی
      await db.query(`DELETE FROM activity_logs WHERE created_at < CURRENT_TIMESTAMP - INTERVAL '30 days'`);
